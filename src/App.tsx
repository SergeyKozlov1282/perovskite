...
function App() {
  ...
  const handleMaterialChange = (layerType: 'perovskite' | 'htl', material: string) => {
  ...
  const createChartData = (type: 'jv' | 'qe' | 'pce') => {
    ...
    datasets.push({
      ...
      data: type === 'jv' ? primaryData.j : type === 'qe' ? primaryData.qe : primaryData.eta,
      ...
      if (config !== currentKey) {
        ...
        if (compData) {
          ...
          datasets.push({
            ...
            data: type === 'jv' ? compData[type].j : type === 'qe' ? compData[type].qe : compData[type].eta,
            borderColor: chartColors[colorIndex % chartColors.length],
            backgroundColor: chartColors[colorIndex % chartColors.length].replace(')', ', 0.1)'),
            fill: false,
            borderDash: [5, 5],
            borderWidth: 2,
            tension: 0.1
    ...
    return {
      labels: type === 'jv' ? primaryData.v.map(v => v.toFixed(2)) : 
              type === 'qe' ? primaryData.lambda : 
      ...
      <main className="container mx-auto px-6 py-8">
        ...
        >
          ...
          <div className="bg-white/80 backdrop-blur-sm p-8 rounded-2xl shadow-xl border border-white/20">
            ...
            >
              Используйте данный инструмент для анализа и сравнения электрофизических характеристик 
              различных конфигураций перовскитных солнечных элементов. Кликните на слой в схеме, 
              чтобы выбрать материал, обновить графики и ключевые показатели.
            </motion.p>
            ...
            >
              <motion.button
                ...
                className="inline-flex items-center space-x-2 px-6 py-3 bg-primary-500 text-white rounded-lg hover:bg-primary-600 transition-colors font-medium shadow-lg"
                whileHover={{ scale: 1.05, y: -2 }}
                whileTap={{ scale: 0.95 }}
              >
                <Settings className="w-5 h-5" />
                <span>Показать параметры слоёв</span>
              </motion.button>
            </motion.div>
