...
const LayerStructure: React.FC<LayerStructureProps> = ({ 
  ...
  const materials = {
    perovskite: [
      { value: 'MAPbI3', label: 'MAPbI₃ (CH₃NH₃PbI₃)', color: 'bg-red-100' },
      { value: 'CsPbI3', label: 'CsPbI₃', color: 'bg-red-200' }
    ],
    htl: [
  ...
  const handleMaterialSelect = (layerType: 'perovskite' | 'htl', material: string) => {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-gray-500 mb-4 text-center">
        Кликните на слой для выбора материала
      </p>
      ...
      <div className="space-y-3 text-center w-full max-w-sm relative">
        ...
        >
          <span className="font-medium">Прозрачный электрод (FTO/ITO)</span>
        </motion.div>
        ...
        <div className="relative">
          <motion.div 
            className={`p-4 rounded-lg shadow-sm cursor-pointer transition-all duration-200 ${
              materials.perovskite.find(m => m.value === perovskite)?.color || 'bg-red-100'
              ...
              >
                {materials.perovskite.map((material) => (
                  <motion.div
                    ...
                    onClick={() => handleMaterialSelect('perovskite', material.value)}
                  >
                    {material.label}
                  </motion.div>
                ))}
        ...
        <div className="relative">
          <motion.div 
            className={`p-4 rounded-lg shadow-sm cursor-pointer transition-all duration-200 ${
              materials.htl.find(m => m.value === htl)?.color || 'bg-green-100'
      ...
      <motion.p 
        ...
        animate={{ opacity: 1 }}
      >
        Текущая конфигурация: <span className="font-medium text-primary-600">
          {getCurrentLabel('perovskite', perovskite).split(' ')[0]} / {getCurrentLabel('htl', htl).split(' ')[0]}
        </span>
      </motion.p>
    </motion.div>
  );
